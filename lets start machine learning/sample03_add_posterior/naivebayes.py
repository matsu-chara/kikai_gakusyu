...
#文字化け回避用 dictが化けるので展開して表示
def print_posterior(words):
	post = nb.posterior(words)
	for cat in post.keys():
		print u'%s:%.4f' % (cat , nb.posterior(words)[cat])
...
class NaiveBayes:
	...
	normalize = 0
        ...
        for cat in self.catcount.keys():
            ln_prob = self.score(word, cat)
	    prob = math.exp(ln_prob)
	    post_prob[cat]=prob
	    normalize += post_prob[cat]

	#for回さないで何かないのか
	for cat in self.catcount.keys():
		post_prob[cat]/=normalize
...
if __name__ == "__main__":
	...
	Python（パイソン）は、オランダ人のグイド・ヴァンロッサムが作ったオープンソースのプログラミング言語。オブジェクト指向スクリプト言語の一種であり、Perlとともに欧米で広く普及している。
			...
			Pythonは英語で爬虫類のニシキヘビの意味で、Python言語のマスコットやアイコンとして使われることがある。
			Pythonは汎用の高水準言語である。
			プログラマの生産性とコードの信頼性を重視して設計されており、核となるシンタックスおよびセマンティクスは必要最小限に抑えられている反面、利便性の高い大規模な標準ライブラリを備えている。
			Unicodeによる文字列操作をサポートしており、日本語処理も標準で可能である。
